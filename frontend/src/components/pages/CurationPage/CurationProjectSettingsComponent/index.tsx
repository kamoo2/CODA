...
import { CurationProjectDto, ProjectFileDto } from '@/types/analysis';
...
interface Props {
  selectedProject: CurationProjectDto;
  onUpdateDescription: (description: string) => void;
  onDeleteProjectFile: () => void;
}

export const CurationProjectSettingsComponent = (props: Props) => {
  ...
  const updateDescription = async (projectId: string, description: string) => {
    await curationService.updateProjectDescription(projectId, description);
  };

  // description text area unfocus 시, 데이터 저장 함수
  const handleBlur = async (event: React.FocusEvent<HTMLTextAreaElement>) => {
    const description = event.target.value;
    await curationService.updateProjectDescription(props.selectedProject.id, description);
  };
  ...
  const saveProjectFile = async (newFile: ProjectFileDto) => {
    await curationService.saveProjectFile(newFile);
  };
  ...
  const handleRemoveFile = (fileId: string) => {
    setFiles((prev) => prev.filter((data) => data.uploadFileId !== fileId));
    // Curation Component 의 조건도 지우기 때문에, 해당 컴포넌트에 조건이 지워짐을 알려서 재랜더링을 발생시켜야함.
    deleteEvalProjectFile(fileId);
  };

  const deleteEvalProjectFile = async (fileId: string) => {
    await curationService.deleteProjectFile(props.selectedProject.id, fileId);
    props.onDeleteProjectFile();
  };
